...
export default function Home(props: HomeProps) {
  ...
  return (
    <div className='max-w-[1124px] h-screen mx-auto grid grid-cols-2
    gap-28 items-center'>
      <main>
        <Image src={logoImg}
          alt="NLW COPA"
        ...
        <form onSubmit={createPool} className='mt-10 flex gap-2'>
          <input
            className='fle x-1 px-6 py-4 rounded bg-gray-800 border border-gray-600 text-sm text-gray-100'
            type="text"
            required
        ...
        <p className='mt-4 font-sm text-gray-300 leading-relaxed'>
...
export const getServerSideProps = async () => {
  ...
  const [
    pollCountResponse, guessCountResponse, usersCountResponse
  ] = await Promise.all([
    api.get('pools/count'),
